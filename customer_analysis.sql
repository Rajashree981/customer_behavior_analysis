...
--which are the top 5 products with highst average review rating?
select item_purchased, round(AVG(review_rating::numeric),2) as "Average product rating"
from customer group by item_purchased
order by avg(review_rating) desc limit 5;

--compare avg purchase amounts btwn std and express shipping
...
--btwn subscribers and non-subscribers
...
--which 5 products have the highest % of purchase with discounts applied
select item_purchased,
round(100*sum(case when discount_applied='Yes' then 1 else 0 end)/count(*),2) as discount_rate
from customer group by item_purchased
order by discount_rate desc limit 5;

--segment customers into new, returning, loyal based on their total no. of previous purcahses, 
...
--wht is the top 3 most purchased products within each category?
with item_counts as (
...
--are customers who are repeat buyers more thn 5 previous purchases also likely to subscribe?
select subscription_status, 
count(customer_id)as repeat_buyers
from customer where previous_purchases>5
group by subscription_status; 

--wht is the revenue contribution of each age group
select age_group, 
